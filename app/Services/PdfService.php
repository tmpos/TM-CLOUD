...
final class PdfService
{
    ...
    public function invoiceHtml(array $project, array $invoice): string
    {
        ...
        $signature = $this->signatures?->status($project, $invoice, true) ?? ['status' => 'none'];
        ...
        return '<!doctype html><html lang="es"><head><meta charset="utf-8"><title>' . $documentTitle . ' ' . $invoiceNumber . '</title><style>'
            ...
            . '<table class="lower"><tr><td class="terms"><div class="panel-title">TÉRMINOS Y OBSERVACIONES</div><p>' . ($notes !== '' ? nl2br($e($notes)) : 'Gracias por su compra. Conserve este documento para futuras referencias.') . '</p></td><td class="qr-cell">' . $qrBlock . '</td><td class="signature">' . $signatureHtml . '</td></tr></table></section>'
