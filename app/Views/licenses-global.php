...
<div class="mb-4 flex justify-end"><button data-dialog-open="#new-license-global-dialog" class="btn-primary">New license</button></div>
<div class="table-wrap"><table class="data-table"><thead><tr><th>Project</th><th>Name</th><th>System</th><th>Status</th><th>Type</th><th>Expires</th><th>Actions</th></tr></thead><tbody>
<?php foreach ($licenses as $license): ?><tr><td><a class="font-semibold text-brand" href="/projects/<?= e($license['project_uid']) ?>"><?= e($license['project_name']) ?></a></td><td class="text-white"><?= e($license['nombre'] ?: $license['system_name']) ?></td><td class="text-slate-400 text-xs"><?= e($license['system_name']) ?></td>
...
<button class="btn-secondary text-xs px-2 py-1.5" data-dialog-open="#key-license-<?= e($license['uid']) ?>">License</button>
<button class="btn-secondary text-xs px-2 py-1.5" data-dialog-open="#creds-license-<?= e($license['uid']) ?>">Keys</button>
<?php if ($license['status'] === 'active'): ?><form method="post" action="/licenses/<?= e($license['uid']) ?>/status" class="inline"><input type="hidden" name="_csrf" value="<?= e(Csrf::token()) ?>"><input type="hidden" name="status" value="blocked"><button class="btn-danger text-xs px-2 py-1.5">Block</button></form><?php endif; ?>
<?php if ($license['status'] === 'blocked'): ?><form method="post" action="/licenses/<?= e($license['uid']) ?>/status" class="inline"><input type="hidden" name="_csrf" value="<?= e(Csrf::token()) ?>"><input type="hidden" name="status" value="active"><button class="btn-secondary text-xs px-2 py-1.5">Unblock</button></form><?php endif; ?>
<?php if ((int)$license['current_uses'] > 0): ?><form method="post" data-confirm="Reset usage counter?" action="/licenses/<?= e($license['uid']) ?>/reset-uses" class="inline"><input type="hidden" name="_csrf" value="<?= e(Csrf::token()) ?>"><button class="btn-secondary text-xs px-2 py-1.5">Reset uses</button></form><?php endif; ?>
<form method="post" data-confirm="Delete this license permanently?" action="/licenses/<?= e($license['uid']) ?>/delete" class="inline"><input type="hidden" name="_csrf" value="<?= e(Csrf::token()) ?>"><button class="btn-danger text-xs px-2 py-1.5">Delete</button></form>
...
<dialog id="new-license-global-dialog" class="w-full max-w-2xl rounded-2xl border border-line bg-panel p-0 text-slate-200"><form class="max-h-[90vh] overflow-y-auto p-6" method="post" action="/licenses"><input type="hidden" name="_csrf" value="<?= e(Csrf::token()) ?>"><div class="mb-5 flex justify-between"><h3 class="text-lg font-semibold text-white">New license</h3><button type="button" data-dialog-close class="text-slate-500">Close</button></div><div class="grid gap-4 md:grid-cols-2">
<label><span class="label">Project</span><select class="input" name="project_uid" required><?php foreach ($projects as $project): ?><option value="<?= e($project['uid']) ?>"><?= e($project['name']) ?></option><?php endforeach; ?></select></label>
