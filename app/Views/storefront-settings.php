...
<aside class="space-y-5">
    <section class="card p-5"><h3 class="font-semibold text-white">Páginas informativas</h3><p class="mt-2 text-sm text-slate-500">Completa las políticas con las condiciones reales de tu negocio. Los textos se comparten con Configuración → Página web de TMPOS.</p>
    <label class="mt-4 block"><span class="label">Horario de atención</span><textarea class="input" name="contact_hours" rows="3" maxlength="16000"><?= e($store['contact_hours'] ?? '') ?></textarea></label>
    <?php foreach (\App\Services\StorefrontPagesService::PAGES as $pageSlug => $definition): if ($definition['field'] === '') continue; ?>
    <details class="mt-4"><summary class="cursor-pointer font-semibold"><?= e($definition['title']) ?></summary><label class="mt-3 block"><span class="label">Contenido de <?= e($definition['title']) ?></span><textarea class="input" name="<?= e($definition['field']) ?>" rows="8" maxlength="16000"><?= e($store[$definition['field']] ?? '') ?></textarea></label><a class="text-sm underline" href="<?= e($store['url'] . '/pages/' . $pageSlug) ?>" target="_blank" rel="noopener">Ver página publicada</a></details>
    <?php endforeach; ?></section>
    <section class="card p-5"><h3 class="font-semibold text-white">Catálogo</h3><input type="hidden" name="show_prices" value="0"><label class="mt-4 flex items-center gap-2 text-sm"><input type="checkbox" name="show_prices" value="1" <?= ($store['show_prices']??1)?'checked':'' ?>> Mostrar precios</label><p class="mt-2 text-xs text-slate-500">Sin precios, la tienda funciona como catálogo y no permite compras online.</p><label class="mt-4 block"><span class="label">Almacén visible</span><select class="input" name="warehouse_uid"><option value="">Primero por defecto<?= !empty($warehouses)?' — '.e($warehouses[0]['name']):'' ?></option><?php foreach($warehouses as $warehouse): ?><option value="<?= e($warehouse['uid']) ?>" <?= ($store['warehouse_uid']??'')===$warehouse['uid']?'selected':'' ?>><?= e($warehouse['name']) ?></option><?php endforeach; ?></select></label><label class="mt-4 block"><span class="label">Tipo de negocio</span><select class="input" name="business_type"><?php foreach(['auto'=>'Detectar automáticamente','electronics'=>'Electrónica y celulares','restaurant'=>'Restaurante y comida','general'=>'Tienda general'] as $value=>$label): ?><option value="<?= e($value) ?>" <?= ($store['business_type']??'auto')===$value?'selected':'' ?>><?= e($label) ?></option><?php endforeach; ?></select><small class="text-slate-500">La detección automática usa las tablas y el nombre del proyecto.</small></label><label class="mt-4 block"><span class="label">Tabla de productos</span><select class="input" name="catalog_table"><option value="">Detectar automáticamente</option><?php foreach($tables as $table): ?><option value="<?= e($table) ?>" <?= $store['catalog_table']===$table?'selected':'' ?>><?= e($table) ?></option><?php endforeach; ?></select></label><label class="mt-4 block"><span class="label">Moneda</span><select class="input" name="currency"><?php foreach(['DOP'=>'Peso dominicano','USD'=>'Dólar estadounidense','EUR'=>'Euro'] as $code=>$label): ?><option value="<?= $code ?>" <?= $store['currency']===$code?'selected':'' ?>><?= e($label) ?></option><?php endforeach; ?></select></label></section>
    <section class="card p-5"><h3 class="font-semibold text-white">Contacto y redes</h3><div class="mt-4 space-y-4"><label><span class="label">Teléfono</span><input class="input" name="phone" value="<?= e($store['phone']) ?>"></label><label><span class="label">WhatsApp</span><input class="input" name="whatsapp" value="<?= e($store['whatsapp']) ?>" placeholder="18095550101"></label><label><span class="label">Correo</span><input class="input" type="email" name="email" value="<?= e($store['email']) ?>"></label><label><span class="label">Dirección</span><textarea class="input min-h-20" name="address"><?= e($store['address']) ?></textarea></label><label><span class="label">Mapa de ubicación</span><textarea class="input" name="map_embed_url" rows="3" maxlength="16000"><?= e($store['map_embed_url'] ?? '') ?></textarea><small>Google Maps: Compartir &rarr; Insertar un mapa &rarr; Copiar HTML. Pega el contenido aquí. Vacío oculta el mapa.</small></label><label><span class="label">Instagram</span><input class="input" type="url" name="instagram_url" value="<?= e($store['instagram_url']) ?>"></label><label><span class="label">Facebook</span><input class="input" type="url" name="facebook_url" value="<?= e($store['facebook_url']) ?>"></label><label><span class="label">Texto del pie</span><textarea class="input min-h-20" name="footer_text"><?= e($store['footer_text']) ?></textarea></label></div></section>
