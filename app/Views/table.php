...
?>
<div class="mb-5 flex flex-wrap items-center justify-between gap-4"><div><a href="/projects/<?= e($project['uid']) ?>" class="text-sm text-slate-500 hover:text-brand">&lt;- <?= e($project['name']) ?></a><h2 class="mt-2 font-mono text-xl font-bold text-white"><?= e($table) ?></h2></div><div class="flex flex-wrap items-center gap-2"><button data-dialog-open="#import-dialog" class="btn-secondary">Import</button><form method="get" action="/projects/<?= e($project['uid']) ?>/tables/<?= e($table) ?>/export" class="flex items-center gap-1.5" title="Deja las fechas vacias para exportar todos los registros"><input type="hidden" name="format" value="csv"><input class="input px-2 py-1.5 text-xs" type="date" name="from" title="Desde"><span class="text-xs text-slate-500">-</span><input class="input px-2 py-1.5 text-xs" type="date" name="to" title="Hasta"><button class="btn-secondary text-xs px-2 py-1.5">Export CSV</button></form><button data-dialog-open="#record-dialog" class="btn-primary">Add record</button></div></div>
