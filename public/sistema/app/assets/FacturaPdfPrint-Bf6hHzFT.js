...
</html>`}async function ce(e,t){P.value&&URL.revokeObjectURL(P.value),P.value=e,z.value=t,M.value=!0}function $(){P.value&&URL.revokeObjectURL(P.value),P.value=``,z.value=``,M.value=!1}async function le(){if(!P.value)return;let e=`data:application/pdf;base64,${Y(await(await(await fetch(P.value)).blob()).arrayBuffer())}`,t=await window.electron.invoke(`save:pdf`,e,z.value);t.success?A.add({severity:`success`,summary:`Guardado`,detail:`PDF descargado`,life:2e3}):A.add({severity:`error`,summary:`Error`,detail:t.error||`No se pudo guardar el PDF`,life:3e3})}async function ue(e){N.value=!0;try{let t=null;if(!C(e)&&e?.uid)try{t=await F(e)}catch{A.add({severity:`warn`,summary:`Firma no disponible`,detail:`El PDF se generará sin firma porque no se pudo consultar el servidor.`,life:4500})}let n=await Q({factura:e,signature:t}),r=`Factura_${e?.no_factura||`sin_numero`}.pdf`,i=await window.electron.invoke(`generate:pdf`,n,r);if(i.success&&i.dataUrl){let e=await(await fetch(i.dataUrl)).blob();await ce(URL.createObjectURL(e),r)}else A.add({severity:`error`,summary:`Error`,detail:i.error||`No se pudo generar el PDF`,life:3e3})}catch(e){A.add({severity:`error`,summary:`Error`,detail:e.message||`Error al generar PDF`,life:3e3})}finally{N.value=!1}}return u({printFactura:ue,generateFacturaHtml:Q}),(e,t)=>(i(),o(c,null,[a(`div`,I,[s(n(v))]),s(n(S),{visible:M.value,"onUpdate:visible":t[0]||=e=>M.value=e,header:`Vista Previa - Factura PDF`,modal:``,style:{width:`80vw`,height:`90vh`},draggable:!1,onHide:$},{footer:r(()=>[s(n(_),{label:`Cerrar`,severity:`secondary`,text:``,onClick:$}),s(n(_),{label:`Descargar PDF`,icon:`pi pi-download`,onClick:le})]),default:r(()=>[a(`div`,L,[P.value?(i(),o(`iframe`,{key:0,src:P.value,class:`w-full flex-1 border-0 rounded-lg`,style:{"min-height":`70vh`},title:`Factura PDF`},null,8,R)):l(``,!0)])]),_:1},8,[`visible`])],64))}});export{F as i,N as n,P as r,z as t};