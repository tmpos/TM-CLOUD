import{r as e}from"./rolldown-runtime-hePW80VL.js";import{_ as t,k as n}from"./reactivity.esm-bundler-CoVOkjw3.js";import{K as r,N as i,c as a,d as o,g as s,r as c,u as l,v as u}from"./runtime-core.esm-bundler-rmv8K8AY.js";import{f as d,l as f,o as p,t as m,u as h,v as g}from"./tmCloudClient-B4EM5bWa.js";import{S as _,X as v,d as y,k as b,s as x,w as S}from"./index-gHxY8V9d.js";import{t as C}from"./browser-Cpc4qo6y.js";import{a as w,i as T,n as E,o as D,r as O}from"./TicketFacturaPrint-DRwpVqJi.js";import{n as k}from"./funciones-BvWR8JyV.js";var A=e(C());function j(e){return e?.data?.signature||e?.data?.request||e?.data||e}function M(e){return JSON.parse(JSON.stringify(e??{}))}function N(e){let t=j(e),n=String(t?.url||``).trim();if(!/^https:\/\//i.test(n))throw Error(`El servidor no devolvió un enlace HTTPS de firma válido`);return{uid:String(t?.uid||``),url:n,expiresAt:t?.expires_at||t?.expiresAt||null,status:t?.status===`signed`?`signed`:`pending`}}function P(e){let t=j(e),n=[`none`,`pending`,`signed`,`stale`,`expired`,`unavailable`].includes(t?.status)?t.status:`none`,r=String(t?.signature_data_uri||t?.signatureDataUri||``).trim();return{status:n,signerName:String(t?.signer_name||t?.signerName||``).trim(),signedAt:t?.signed_at||t?.signedAt||null,imageDataUri:n===`signed`&&/^data:image\/png;base64,[A-Za-z0-9+/=]+$/.test(r)?r:null}}function F(e,t){return[`Hola ${e?.nombre_cliente||`cliente`},`,`Por favor revisa y firma la factura ${e?.no_factura||``} por RD$${Number(e?.total||0).toFixed(2)}.`,`Abre este enlace seguro para firmar:`,t,``,`La factura mostrará tu firma después de confirmarla.`].join(`
`)}async function I(e){let t=String(e?.uid||``).trim();if(!t)throw Error(`La factura aún no tiene UID de sincronización`);if(window.__isServerSystem||window.__isElectron){let n=await window.electron.invoke(`facturas:crearEnlaceFirma`,{record_uid:t,invoice:M(e)});if(n?.success===!1)throw Error(n.error||`No se pudo crear el enlace de firma`);return N(n)}await p(!0);let n=f();if(!n?.url||!n.key)throw Error(`TM Cloud no está configurado para emitir solicitudes de firma`);let r=await fetch(`${n.url}/invoices/${encodeURIComponent(t)}/signature-request`,{method:`POST`,headers:m(n.key,!0),body:JSON.stringify({expires_at:new Date(Date.now()+6048e5).toISOString().replace(`T`,` `).slice(0,19)})});if(!r.ok)throw Error(await g(r));return N(await r.json())}async function L(e){let t=String(e?.uid||``).trim();if(!t)return{status:`none`,signerName:``,signedAt:null,imageDataUri:null};if(window.__isServerSystem||window.__isElectron){let n=await window.electron.invoke(`facturas:obtenerFirma`,{record_uid:t,invoice:M(e)});if(n?.success===!1)throw Error(n.error||`No se pudo consultar la firma`);return P(n)}await p(!0);let n=f();if(!n?.url||!n.key)throw Error(`TM Cloud no está configurado para consultar firmas`);let r=await fetch(`${n.url}/invoices/${encodeURIComponent(t)}/signature`,{headers:m(n.key)});if(!r.ok)throw Error(await g(r));return P(await r.json())}var R={style:{display:`none`}},z={class:`flex flex-col h-full gap-3`},B=[`src`],V=u({__name:`FacturaPdfPrint`,setup(e,{expose:u}){function f(e){let t=O(),n=V(e?.otro,{}),r=e.banco_nombre||n?.banco_nombre||``,i=[],a=Number(e.efectivo||0);a>0&&i.push({tipo:`efectivo`,etiqueta:t.cash,monto:a});let o=V(e.tarjeta_mixta,e.tarjeta_mixta),s=o&&typeof o==`object`?o:n?.tarjeta_mixta||{},c=Number(e.tarjeta||s?.total||Number(s?.monto_base||0)+Number(s?.monto_comision||0));c>0&&i.push({tipo:`tarjeta`,etiqueta:t.card,monto:c});let l=V(e.transferencias_mixtas,e.transferencias_mixtas),u=V(n?.transferencias_mixtas,n?.transferencias_mixtas),d=Array.isArray(l)?l:Array.isArray(u)?u:Number(e.transferencia)>0?[{monto:e.transferencia,banco_nombre:r}]:[];for(let e of d){let n=Number(e?.monto||0);if(n<=0)continue;let r=e?.banco_nombre?` (${e.banco_nombre})`:``;i.push({tipo:`transferencia`,etiqueta:`${t.transfer}${r}`,monto:n})}if(Number(e.cheque)>0&&i.push({tipo:`cheque`,etiqueta:t.check,monto:Number(e.cheque)}),a<=0){let n=Number(e.total||0),r=i.reduce((e,t)=>e+t.monto,0),a=Math.round((n-r)*100)/100;a>.009&&i.unshift({tipo:`efectivo`,etiqueta:t.cash,monto:a})}return i}function m(e){let t=O(),n=String(e.metodo_pago||``).toUpperCase(),r=V(e?.otro,{}),i=e.banco_nombre||r?.banco_nombre||``;if(n!==`MIXTO`){let t=D(e.metodo_pago);return i?`${t} - ${i}`:t}return t.mixed}function g(e){let t=O(),n={efectivo:t.cash,tarjeta:t.card,transferencia:t.transfer,cheque:t.check},r=[...new Set(f(e).map(e=>n[e.tipo].toUpperCase()))];return r.length?`${r.join(` Y `)} - ${t.mixed}`:t.mixed}function C(e){return String(e.tipo_factura||``).toLowerCase()===`cotizacion`||String(e.estado_factura||``).toLowerCase()===`cotizacion`}let j=v(),M=y(),N=t(!1),P=t(!1),F=t(``),I=t(``);function V(e,t){if(e==null)return t;if(typeof e==`string`)try{return JSON.parse(e)}catch{return t}return e}function H(e,t=0){let n=Number(e);return Number.isFinite(n)?n:t}function U(e,t={}){let n=V(e?.otro,{}),r=n?.alanube_response||e?.alanube_response||{};return{documentStampUrl:t?.document_stamp_url||e?.document_stamp_url||e?.documentStampUrl||n?.documentStampUrl||n?.document_stamp_url||r?.documentStampUrl||r?.document_stamp_url||``,securityCode:t?.security_code||e?.codigo_seguridad||e?.securityCode||n?.securityCode||n?.security_code||r?.securityCode||r?.security_code||``,legalStatus:t?.legal_status||e?.alanube_legal_status||r?.legalStatus||n?.legalStatus||``,status:t?.status||e?.alanube_status||r?.status||n?.status||``}}async function W(e){if(e?.id)try{let t=await window.db.getWhere(`facturas_ecf`,`factura_id = ?`,[e.id]),n=t?.success&&Array.isArray(t.data)?t.data[0]:null;if(n)return U(e,n)}catch{}return U(e)}function G(e){return x(e)}function K(e,t=``){let n=String(t||``).trim().match(/^(\d{1,2}:\d{2})/),r=String(e||``).trim(),i=r.match(/(?:T|\s)(\d{1,2}:\d{2})/),a=n?n[1].padStart(5,`0`):i?i[1].padStart(5,`0`):``,o=r.match(/^(\d{4})-(\d{2})-(\d{2})/),s=r.match(/^(\d{2})\/(\d{2})\/(\d{4})/);if(o)return`${o[3]}/${o[2]}/${o[1]}${a?` ${a}`:``}`;if(s)return`${s[1]}/${s[2]}/${s[3]}${a?` ${a}`:``}`;let c=e instanceof Date?e:new Date(e);if(Number.isNaN(c.getTime()))return a;let l=`${String(c.getDate()).padStart(2,`0`)}/${String(c.getMonth()+1).padStart(2,`0`)}/${c.getFullYear()}`,u=`${String(c.getHours()).padStart(2,`0`)}:${String(c.getMinutes()).padStart(2,`0`)}`;return`${l} ${a||u}`}function q(e){return[e?.imei,e?.lista_imei,e?.imeis,e?.serial,e?.seriales].flatMap(e=>Array.isArray(e)?e:typeof e==`string`?e.split(`,`):e?[e]:[]).map(e=>typeof e==`object`?String(e.imei||e.serial||``).trim():String(e).trim()).filter(Boolean).filter((e,t,n)=>n.indexOf(e)===t)}function ee(e){return String(e?.codigo||e?.codigo_barra||e?.cod_producto||e?.sku||e?.referencia||e?.barcode||e?.imei||e?.serial||e?.accesorio_id||e?.telefono_id||e?.imei_id||e?.serial_id||``).trim()}function J(e,t=``){let n=String(e??``).trim();return n?/^(data:|https?:\/\/|file:|blob:)/i.test(n)?n:n.startsWith(`/`)?t?`${t.replace(/\/$/,``)}${n}`:n:t?`${t.replace(/\/$/,``)}/${n.replace(/^\.\//,``)}`:n:``}function Y(e){let t=new Uint8Array(e),n=``,r=32768;for(let e=0;e<t.length;e+=r)n+=String.fromCharCode(...t.subarray(e,e+r));return btoa(n)}async function te(e,t=``){let n=String(e??``).trim();if(!n)return``;let r=J(n,t);if(/^data:/i.test(r))return r;if(!/^(https?:\/\/|file:|blob:|\/)/i.test(n))try{await p();let e=d(n),t=h()?.key||``;if(e){let n=await fetch(e,{headers:t?{Authorization:`Bearer ${t}`}:{}});if(n.ok)return`data:${n.headers.get(`content-type`)||`image/png`};base64,${Y(await n.arrayBuffer())}`}}catch{}return r}async function ne(){try{let e=await window.db.getAll(`empresa`);if(e.success&&e.data?.length>0)return e.data[0]}catch{}return{}}async function re(e){try{let t=await window.db.getAll(`clientes`);if(!t.success||!Array.isArray(t.data))return{};let n=String(e.cod_cliente||e.cliente_id||``).trim(),r=String(e.nombre_cliente||e.cliente||e.comprador||``).trim().toUpperCase(),i=String(e.telefono_cliente||e.telefono||e.whatsapp||``).trim(),a=String(e.rnc_cliente||e.cedula_cliente||e.rnc||e.cedula||``).trim();return t.data.find(e=>String(e.id||``)===n||String(e.codigo||``)===n||String(e.nombre||``).trim().toUpperCase()===r||String(e.telefono||``).trim()===i||String(e.whatsapp||``).trim()===i||String(e.rnc||``).trim()===a||String(e.cedula||``).trim()===a)||{}}catch{return{}}}function ie(e,t={}){return{nombre:e.nombre_cliente||e.cliente||e.comprador||t?.nombre||`CONSUMIDOR FINAL`,telefono:e.telefono_cliente||e.telefono||e.whatsapp||t?.telefono||t?.whatsapp||``,documento:e.rnc_cliente||e.cedula_cliente||e.rnc||e.cedula||t?.rnc||t?.cedula||``,direccion:e.direccion_cliente||e.direccion||t?.direccion||``}}function ae(e){return String(e.nota||e.observacion||e.observaciones||e.nota_factura||e.comentario||``).trim()}async function oe(){try{return await k(`datosarchivo`)}catch{return{}}}async function se(){try{let e=await window.db.getAll(`impresoras_config`);return e?.success&&Array.isArray(e.data)&&e.data[0]||{}}catch{return{}}}function X(e,t,n,r){let i=Number(e);return Number.isFinite(i)?Math.min(r,Math.max(n,i)):t}function Z(e){return String(e||``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]||e)}async function Q({factura:e,cliente:t=null,datosEmpresa:n=null,signature:r=null}){let i=O(),a=(await oe())?.VITE_LINKURL||``,o=n?.empresa||n?.datosEmpresa?.empresa||e.empresa||await ne(),s=t||await re(e),c=T({factura:e,empresa:o,cliente:s}),l={...ie(e,s),...c.customer};l.nombre=w(l.nombre);let u=(ae(e)||i.thanks).replace(/\n/g,`<br>`),d=c.items,p=await te(o?.logoprinter||o?.logo,a),h=await se(),_=X(h.factura_logo_ancho,150,30,400),v=X(h.factura_logo_alto,90,20,250),y=await W(e),b=y.documentStampUrl||`${a||`https://tmposrd.com`}/receipt/factura?factura=${e.no_factura||``}`,x=e.qr||``;try{x||=await A.toDataURL(b)}catch{}let S=Array.isArray(d)?d.map(e=>{let t=H(e.cantidad??e.quantity,0),n=H(e.precio_final??e.precio_venta??e.precio_unitario??e.precio,0),r=H(e.precio_normal??e.precio_lista??e.precio_venta_normal,n),i=H(e.descuento,0),a=H(e.impuesto_venta??e.impuesto,0),o=H(e.total,n*t-i),s=r>0&&n>=0&&n<r;return{...e,codigoProducto:ee(e),cantidad:t,precioUnidad:n,precioNormal:r,descuento:i,tieneDescuentoProducto:s,impuestoTotal:a*t,totalProducto:o,imeis:q(e),detallesImei:E(e)}}):[],D=H(e.impuesto??e.impuestos)>0||S.some(e=>e.impuestoTotal>0),k=H(e.descuento)>0||S.some(e=>e.descuento>0),j=S.reduce((e,t)=>e+t.impuestoTotal,0)||H(e.impuesto??e.impuestos),N=H(e.total),P=H(e.subtotal,N+H(e.descuento)-j),F=String(e.metodo_pago||``).toUpperCase()===`MIXTO`,I=F?N:P,L=(F?f(e):[]).map(e=>`<tr class="payment-row"><td>${e.etiqueta}</td><td class="text-right">${G(e.monto)}</td></tr>`).join(``),R=5+ +!!D+ +!!k,z=K(e.fecha_emision||e.fecha||``,e.hora||``),B=i.language===`en`?`CUSTOMER DETAILS`:`DATOS DEL CLIENTE`,V=i.language===`en`?`PAYMENT SUMMARY`:`RESUMEN DE PAGO`,U=C(e)?i.quote.toUpperCase():e.metodo_pago===`CREDITO`?i.creditInvoice:i.invoice.toUpperCase(),J=S.map(e=>{let t=e.detallesImei.length?e.detallesImei.map(e=>`<div class="imei-line">IMEI: ${e}</div>`).join(``):e.imeis.length?`<div class="imei-line">IMEI: ${e.imeis.join(`, `)}</div>`:``,n=e.tieneDescuentoProducto?`<div class="discount-line">Normal: <span class="line-through">${G(e.precioNormal)}</span> &nbsp; Con descuento: <strong>${G(e.precioUnidad)}</strong></div>`:``;return`
      <tr class="invoice-line">
        ...
        <td class="text-right">${G(e.precioUnidad)}</td>
        ${D?`<td class="text-right">${G(e.impuestoTotal)}</td>`:``}
        ${k?`<td class="text-right">${G(e.descuento)}</td>`:``}
        <td class="text-right"><strong>${G(e.totalProducto)}</strong></td>
      </tr>
    `}).join(``),Y=Array.from({length:Math.max(0,8-S.length)},()=>`<tr class="invoice-line empty-row">${Array.from({length:R},()=>`<td>&nbsp;</td>`).join(``)}</tr>`).join(``);return`<!DOCTYPE html>
...
<body>
  <div class="page">
    <div class="header">
      <div class="company">
        ...
        <div class="company-copy">
          ...
          <div class="company-meta">
            ${o.legal||o.rnc?`${M.businessIdLabel}: ${o.legal||o.rnc}<br>`:``}
      ...
      <div class="invoice-box">
        <div class="document-heading">
          <div class="document-type">${U}</div>
          <div class="document-number">${e.no_factura||``}</div>
        </div>
        <table>
          <tr><td><strong>${i.date}</strong></td><td class="text-right">${z}</td></tr>
          ${C(e)||!(e.ncf||e.comprobante)?``:`<tr><td><strong>${M.fiscalDocumentLabel}</strong></td><td class="text-right">${e.ncf||e.comprobante||``}</td></tr>`}
        </table>
        <div class="invoice-title">${C(e)?`${i.quote.toUpperCase()} #${e.no_factura||``}`:e.metodo_pago===`CREDITO`?`${i.creditInvoice} #${e.no_factura||``}`:F?g(e):`${i.invoice.toUpperCase()} #${e.no_factura||``}`}</div>
    ...
    <div class="client-box">
      <div class="client-data">
        ...
        <div class="client-grid">
          ...
          <p><strong>${M.customerIdLabel.toUpperCase()}:</strong><br>${l.documento||i.notApplicable}</p>
    ...
    <div class="bottom">
      ...
      <div class="totals">
        <div class="totals-title">${V}</div>
        <table>
          <tr><td>${i.subtotal}</td><td class="text-right">${G(I)}</td></tr>
          ${D?`<tr><td>${M.shortName}</td><td class="text-right">${G(j)}</td></tr>`:``}
          ${k?`<tr><td>${i.discount}</td><td class="text-right">${G(e.descuento)}</td></tr>`:``}
          ${L}
          <tr><td>${i.total}</td><td class="text-right">${G(e.total)}</td></tr>
        </table>
      </div>
    </div>
...
</html>`}async function ce(e,t){F.value&&URL.revokeObjectURL(F.value),F.value=e,I.value=t,N.value=!0}function $(){F.value&&URL.revokeObjectURL(F.value),F.value=``,I.value=``,N.value=!1}async function le(){if(!F.value)return;let e=`data:application/pdf;base64,${Y(await(await(await fetch(F.value)).blob()).arrayBuffer())}`,t=await window.electron.invoke(`save:pdf`,e,I.value);t.success?j.add({severity:`success`,summary:`Guardado`,detail:`PDF descargado`,life:2e3}):j.add({severity:`error`,summary:`Error`,detail:t.error||`No se pudo guardar el PDF`,life:3e3})}async function ue(e){P.value=!0;try{let t=null;if(!C(e)&&e?.uid)try{t=await L(e)}catch{j.add({severity:`warn`,summary:`Firma no disponible`,detail:`El PDF se generará sin firma porque no se pudo consultar el servidor.`,life:4500})}let n=await Q({factura:e,signature:t}),r=`Factura_${e?.no_factura||`sin_numero`}.pdf`,i=await window.electron.invoke(`generate:pdf`,n,r);if(i.success&&i.dataUrl){let e=await(await fetch(i.dataUrl)).blob();await ce(URL.createObjectURL(e),r)}else j.add({severity:`error`,summary:`Error`,detail:i.error||`No se pudo generar el PDF`,life:3e3})}catch(e){j.add({severity:`error`,summary:`Error`,detail:e.message||`Error al generar PDF`,life:3e3})}finally{P.value=!1}}return u({printFactura:ue,generateFacturaHtml:Q}),(e,t)=>(i(),o(c,null,[a(`div`,R,[s(n(b))]),s(n(_),{visible:N.value,"onUpdate:visible":t[0]||=e=>N.value=e,header:`Vista Previa - Factura PDF`,modal:``,style:{width:`80vw`,height:`90vh`},draggable:!1,onHide:$},{footer:r(()=>[s(n(S),{label:`Cerrar`,severity:`secondary`,text:``,onClick:$}),s(n(S),{label:`Descargar PDF`,icon:`pi pi-download`,onClick:le})]),default:r(()=>[a(`div`,z,[F.value?(i(),o(`iframe`,{key:0,src:F.value,class:`w-full flex-1 border-0 rounded-lg`,style:{"min-height":`70vh`},title:`Factura PDF`},null,8,B)):l(``,!0)])]),_:1},8,[`visible`])],64))}});export{L as i,F as n,I as r,V as t};