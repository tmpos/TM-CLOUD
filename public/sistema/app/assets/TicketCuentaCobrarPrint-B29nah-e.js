import{A as e}from"./reactivity.esm-bundler-C6Po7tbM.js";import{P as t,d as n,g as r,v as i}from"./runtime-core.esm-bundler-EoPqhpgY.js";import{B as a,N as o,P as s,q as c,xt as l}from"./index-C5OxJoIX.js";import{t as u}from"./printImageService-BQdZNYcQ.js";var d={style:{display:`none`}},f=i({__name:`TicketCuentaCobrarPrint`,setup(i,{expose:f}){let p=l(),m=c(),h={printer_name:``,paper_width:80,show_logo:1,show_company_name:1,show_legal:1,show_phone:1,show_address:1,show_email:1,show_cliente:1,show_items:1,show_totals:1,show_barcode:1,show_footer:1,show_qr:0,footer_text:`Gracias por su compra`};function g(e){return e===!0||e===1||e===`1`}function _(e,t=0){let n=Number(e);return Number.isFinite(n)?n:t}function v(e){return _(e).toFixed(2)}function y(e){return String(e?.logoprinter||e?.logo||``).trim()}function b(e){if(!e)return``;let t=new Date(e);return isNaN(t.getTime())?e:`${String(t.getDate()).padStart(2,`0`)}/${String(t.getMonth()+1).padStart(2,`0`)}/${t.getFullYear()}`}function x({cuenta:e,empresa:t,monto:n,abonadoTotal:r,saldoRestante:i,ticketConfig:a,productos:c,factura:l}){let u=s(),d=y(t),f=_(a.paper_width,80),p=f===58?230:300,m=f===58?210:250,h=f===58?200:240,x=n>0?`RECIBO DE ABONO`:`ESTADO DE CUENTA`,S=new Date,C=`${S.toLocaleDateString(o().locale)} ${String(S.getHours()).padStart(2,`0`)}:${String(S.getMinutes()).padStart(2,`0`)}`,w=e.fecha_venta?b(e.fecha_venta):``,T=e.fecha_vencimiento?b(e.fecha_vencimiento):``,E=c.length>0?c.map(e=>{let t=e.nombre||e.descripcion||e.producto||`Producto`,n=_(e.cantidad??e.quantity,1),r=_(e.precio_venta??e.precio_unitario??e.precio??e.price),i=_(e.total,r*n);return`<tr>
          <td>${t}</td>
          <td class="centrado">${n}</td>
          <td class="precio">${u}${v(r)}</td>
          <td class="precio"><b>${u}${v(i)}</b></td>
        </tr>`}).join(``):``,D=[];g(a.show_address)&&t.direccion&&D.push(t.direccion);let O=[];g(a.show_phone)&&t.telefono&&O.push(t.telefono),g(a.show_email)&&t.email&&O.push(t.email),O.length&&D.push(O.join(` / `)),g(a.show_legal)&&(t.legal||t.rnc)&&D.push(`RNC: ${t.legal||t.rnc}`);let k=D.length?`<p>${D.join(`<br>`)}</p>`:``,A=l?`Metodo: ${l.metodo_pago||`CREDITO`}<br>
       ${l.vendedor?`Vendedor: ${l.vendedor}<br>`:``}
       ${l.cajero?`Cajero: ${l.cajero}<br>`:``}`:``,j=[];try{j=JSON.parse(e.pagos||`[]`),Array.isArray(j)||(j=[])}catch{j=[]}let M=j.length>0?j.map(e=>`
      <tr>
