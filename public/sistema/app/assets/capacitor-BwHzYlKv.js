import{T as e}from"./tmCloudClient-BXA3SgwJ.js";import{a as t,c as n,d as r,f as i,h as a,handleElectronInvoke as o,i as s,initLicencia as c,l,m as u,n as d,o as f,p,r as m,s as h,t as g,u as _}from"./capacitorElectron-B8V7hGjT.js";async function v(){console.log(`[Capacitor] Inicializando app para Android...`),y(),await a(),window.db||(window.db={getAll:e=>Promise.resolve(h(e)),getCuadres:(e={})=>{let t=h(`cuadres`);if(!t.success)return Promise.resolve(t);let n=(t.data||[]).filter(t=>!e.almacenUid||!t.almacen_uid||String(t.almacen_uid)===e.almacenUid).filter(t=>{let n=String(t.created_at||t.fecha||``).slice(0,10);return(!e.desde||n>=e.desde)&&(!e.hasta||n<=e.hasta)}).sort((e,t)=>String(t.created_at||t.fecha||``).localeCompare(String(e.created_at||e.fecha||``)));return Promise.resolve({success:!0,data:e.limit?n.slice(0,e.limit):n})},getWhere:(e,t,n=[])=>Promise.resolve(_(e,t,n)),getModified:(e,t)=>Promise.resolve(l(e,t)),getById:(e,t)=>Promise.resolve(n(e,t)),insert:(e,t)=>Promise.resolve(r(e,t)),insertMany:(e,t)=>Promise.resolve(i(e,t)),update:(e,t,n)=>Promise.resolve(u(e,t,n)),delete:(e,t)=>{let n=``;try{n=localStorage.getItem(`mr_user_usuario`)||``}catch{}return Promise.resolve(m(e,t,n))},deleteAll:async t=>{if(![`accesorios`,`telefonos`].includes(t))return{success:!1,error:`Tabla no autorizada para borrado total`};let n=``;try{n=localStorage.getItem(`mr_user_usuario`)||``}catch{}let r=String(n).trim().toLowerCase(),i=h(`usuarios`);if(!(i.success&&(i.data||[]).find(e=>{let t=[e.usuario,e.email,e.nombre].some(e=>String(e||``).trim().toLowerCase()===r),n=[`ACTIVADO`,`ACTIVO`].includes(String(e.estado||``).trim().toUpperCase()),i=[e.rol,e.nivel_seguridad].some(e=>String(e||``).trim().toLowerCase()===`soporte`);return t&&n&&i})))return{success:!1,error:`Solo Soporte puede borrar todo el inventario seleccionado`};try{await e(t)}catch(e){return{success:!1,error:e?.message||`No se pudo vaciar la tabla en TM Cloud`}}return s(t,n)},deleteLocalOnly:(e,n)=>Promise.resolve(t(e,n)),truncateLocalOnly:e=>Promise.resolve([`accesorios`,`telefonos`].includes(e)?p(e):{success:!1,error:`Tabla no autorizada para truncado local`}),bitacoraList:e=>Promise.resolve(d(e)),bitacoraDeleteAll:()=>Promise.resolve(g())}),window.config||(window.config={get:e=>Promise.resolve({success:!0,data:localStorage.getItem(`config_${e}`)||``}),set:(e,t)=>(localStorage.setItem(`config_${e}`,String(t??``)),Promise.resolve({success:!0}))}),window.electron||(window.electron={invoke:(e,...t)=>e===`consultaservidor`&&t[0]===`executeSQL`?Promise.resolve(f(t[1])):o(e,...t),send:()=>{},on:()=>{}}),await c(),window.__isElectron=!1,window.__isCapacitor=!0,x(),console.log(`[Capacitor] App inicializada correctamente`)}function y(){typeof Object.hasOwn!=`function`&&(Object.hasOwn=(e,t)=>Object.prototype.hasOwnProperty.call(e,t))}function b(){if(document.documentElement.classList.add(`capacitor-android`),document.getElementById(`android-webview-compatibility`))return;let e=document.createElement(`style`);e.id=`android-webview-compatibility`,e.textContent=`
    .capacitor-android *,
  ...
  `,document.head.appendChild(e)}async function x(){if(navigator.mediaDevices?.getUserMedia)try{(await navigator.mediaDevices.getUserMedia({video:{facingMode:`environment`}})).getTracks().forEach(e=>e.stop()),console.log(`[Permisos] Camara concedido`)}catch{console.log(`[Permisos] Camara no concedido (se pedira al escanear)`)}`Notification`in window&&Notification.permission==="default"&&Notification.requestPermission().then(e=>console.log(`[Permisos] Notificaciones:`,e))}export{b as applyAndroidRenderingCompatibility,v as initCapacitorApp};